    ...
    http:
      routes:
        - name: all
          ...
          middlewares: [cs]
      middlewares:
        cs: {{oidc: {{issuer: "https://id.example", client_id: rproxy, client_secret_file: /nonexistent/s, cookie_secret_file: /nonexistent/c}}}}
"#,
			bp = backend.port()
		),
	)
	...
	assert!(kinds.contains(&serde_json::json!("compress")) && !kinds.contains(&serde_json::json!("oidc")), "{caps}");
